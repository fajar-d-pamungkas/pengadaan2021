              ...
              <div class="card-body table-responsive p-0">
                <table style="text-align:center;" class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$ada->nama_pengadaan}}</td>
                      <td><a target="_blank" href="{{$ada->deskripsi}}"><button class="btn-primary">Lihat Detail</button></a></td>
                      <td style="width:15%;">
                        @if($ada->gambar != "-")
                        <img style="width:70%;" src="{{asset(Storage::url($ada->gambar))}}"></img>
                        <hr>
                        <a class="konfirmasiGambar" href="/hapusGambar/{{$ada->id_pengadaan}}"><button class="btn-danger"><i class="fas fa-trash"></i>Hapus Gambar</button>
                        </a>
                        @endif
                        @if($ada->gambar == "-")
                        <hr>
                        <form method="post" action="/tambahLaporan" enctype="multipart/form-data">
                          {{csrf_field()}}
                          <input type="hidden" name="id_pengadaan" id="id_pengadaan" value="{{$ada->id_pengadaan}}">
                          ...
                          <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i>Upload Gambar
                          </button>
                        </form>
                        @endif
                      </td>
                      <td><span class="tag tag-success">{{number_format($ada->anggaran,0,",",".")}}</span></td>
                      <td>
                        <button data-id_pengadaan="{{$ada->id_pengadaan}}" data-nama_pengadaan="{{$ada->nama_pengadaan}}" data-deskripsi="{{$ada->deskripsi}}" data-anggaran="{{$ada->anggaran}}" data-toggle="modal" data-target="#ubahModal" class="btn btn-info ubah"><i class="fas fa-edit"></i>Edit Data</button>
                        <a href="/hapusPengadaan/{{$ada->id_pengadaan}}" class="konfirmasiData">
                          <button class="btn btn-danger"><i class="fas fa-trash"></i>Hapus Data
                          </button>
                        </a>
                      </td>
                    </tr>
...
function formatRupiah(angka,prefix){
 ...
 $(document).on("click",".konfirmasiGambar",function(event){
...
$(document).on("click",".konfirmasiData",function(event){
  event.preventDefault();
  const url = $(this).attr('href');
